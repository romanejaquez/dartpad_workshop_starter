...
Now let's define an ```enum``` that will map to the corresponding device screen size that we're supporting. Place your enum code above the ```Utils``` class.
...
Now, using that enum value, extract the value corresponding to it from the ```Utils.deviceTypes``` mapping, which returns a ```DeviceDescription``` object, a model class that contains an icon and a label that maps to the matched supported screen type.
...
// to populate the local variables inside this method
...
- a ``Text`` widget, with value of ``label!``, and with ```style``` set to a ``TextStyle`` object also color ```Colors.blueAccent``` and a font size of ``20``.
